...
func TestRepository_UniqueCompound_RocksDB_Delete_DatabaseCleanup(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping RocksDB test in short mode")
	}

	repo, store, err := newTestExchangeRepository(t)
	require.NoError(t, err)
...
func TestRepository_UniqueCompound_RocksDB_Update_NoStaleIndexes(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping RocksDB test in short mode")
	}
...
func TestRepository_UniqueCompound_RocksDB_Update_BothFieldsChange(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping RocksDB test in short mode")
	}
...
func TestRepository_UniqueCompound_RocksDB_Update_ConstraintViolation(t *testing.T) {
	if testing.Short() {
		t.Skip("Skipping RocksDB test in short mode")
	}

	repo, _, err := newTestExchangeRepository(t)
	require.NoError(t, err)
...
func TestRepository_TTL_BasicExpiration_UpdateTTL(t *testing.T) {
	repo, store, err := newTestTTLRepository(t)
	require.NoError(t, err)
...
func newTestExchangeRepository(t *testing.T) (*db.Repository[Exchange], *db.RocksdbStore, error) {
	store := newRocksdbStore(t)
...
func TestRepository_UniqueCompound_RocksDB_CreateAndFind(t *testing.T) {
	repo, _, err := newTestExchangeRepository(t)
	require.NoError(t, err)
...
func TestRepository_UniqueCompound_RocksDB_BulkCreate(t *testing.T) {
	repo, _, err := newTestExchangeRepository(t)
	require.NoError(t, err)
...
func TestRepository_UniqueCompound_RocksDB_ValidationErrors(t *testing.T) {
	repo, _, err := newTestExchangeRepository(t)
	require.NoError(t, err)
...
// MARK: Data-Only Field Tests for RocksDB

func TestDataOnlyFields_RocksDBValidations(t *testing.T) {
	store := newRocksdbStore(t)
	defer store.Close()
...
func TestDataOnlyFields_RocksDBCreateAndRead(t *testing.T) {
	store := newRocksdbStore(t)
...
func TestDataOnlyFields_RocksDBQueryRestrictions(t *testing.T) {
	store := newRocksdbStore(t)
...
func TestDataOnlyFields_RocksDBUpdate(t *testing.T) {
	store := newRocksdbStore(t)

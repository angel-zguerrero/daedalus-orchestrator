...
func newRocksdbStoreForUOW(t *testing.T) *db.RocksdbStore {
	tmpDir := t.TempDir()
...
func TestUnitOfWork_CreateAndCommit(t *testing.T) {
	uow, store, carRepo, fixRepo, notifRepo, err := newTestUOWDefaultIdGenerator(t)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnitOfWork_TTLBulkCreateAndExpire(t *testing.T) {
	uow, store, _, _, notifRepo, err := newTestUOWDefaultIdGenerator(t)
	require.NoError(t, err)
...
func TestUnitOfWork_MassiveMixedEntities(t *testing.T) {
	uow, store, carRepo, fixRepo, notifRepo, err := newTestUOWDefaultIdGenerator(t)
	require.NoError(t, err)

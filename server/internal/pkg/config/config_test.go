...
func TestConfigFromMapToConfig_AllFieldsCopied(t *testing.T) {
	...
	expected := &Config{
		...
		ReplicaID:           123,
		Roles:               "scheduler,connector",
		SelfMemberHost:      "127.0.0.1",
		ClusterBasePort:     7001,
	...
	assert.Equal(t, expected.SelfMemberHost, actual.SelfMemberHost, "SelfMemberHost should match")
	assert.Equal(t, expected.ClusterBasePort, actual.ClusterBasePort, "ClusterBasePort should match")

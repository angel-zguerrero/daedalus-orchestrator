...
func setupTestExchange(t *testing.T, store db.KVStore, cf, cfs string, exchangeType models.ExchangeType, now time.Time) *models.Exchange {
	uow := db.NewUnitOfWork(store, nil)
	idFactory := &db.DeterministicIDGeneratorFactory{}
	...
	exchange := &models.Exchange{
		ID:         "exchange-id-" + string(exchangeType),
		Code:       "email-events-" + string(exchangeType),
		Name:       "Email Events",
		Type:       exchangeType,
		VNamespace: "default",
		CreatedAt:  now,
...
func setupTestBinding(t *testing.T, store db.KVStore, cf, cfs string, exchange *models.Exchange, queue *models.Queue, routingKey string, now time.Time) *models.Binding {
	uowVN := db.NewUnitOfWork(store, nil)
	vnRepo, err := db.NewVNamespaceRepository(uowVN, &db.DefaultIDGeneratorFactory{}, cf, cfs)
	require.NoError(t, err)
	existingVN, err := vnRepo.GetVNamespaceByName("default", now)
	require.NoError(t, err)
	if existingVN == nil {
		_, err := vnRepo.CreateVNamespace(&models.VNamespace{ID: "vn-default", Name: "default"}, now)
		require.NoError(t, err)
		require.NoError(t, uowVN.Commit())
	}
	...
	var pattern string
	var rk string
	if exchange.Type == models.Topic {
		pattern = routingKey
	} else if exchange.Type == models.Direct {
		rk = routingKey
	}

	assertCmd := &bindingCommand.AssertBindingCommand{
		NewBindingID: "binding-" + exchange.ID + "-" + queue.ID,
		Code:         "binding-" + exchange.ID + "-" + queue.ID,
	...
	res := assertCmd.Execute(uow, now)
	require.Empty(t, res.Error)
	require.NoError(t, uow.Commit())

	b := res.Result.(models.Binding)
	return &b
}
...
func TestScheduledJob_ExchangeFanoutTracking(t *testing.T) {
	store := newTestPebbleStore(t)
	now := time.Date(2025, 1, 1, 10, 0, 0, 0, time.UTC)
	idFactory := &db.DeterministicIDGeneratorFactory{}

	// Setup exchange and 2 target queues
	exchange := setupTestExchange(t, store, TestFC, TestCFS, models.Fanout, now)

	uowQ1 := db.NewUnitOfWork(store, nil)
	qRepo1, _ := db.NewQueueRepository(uowQ1, idFactory, TestFC, TestCFS)
	q1 := &models.Queue{
		ID:                        "queue-fanout-1",
		Code:                      "fanout-queue-1",
		Name:                      "Fanout Queue 1",
	...
	qID1, _ := qRepo1.CreateQueue(q1, now)
	q1.ID = qID1
	require.NoError(t, uowQ1.Commit())

	uowQ2 := db.NewUnitOfWork(store, nil)
	qRepo2, _ := db.NewQueueRepository(uowQ2, idFactory, TestFC, TestCFS)
	q2 := &models.Queue{
		ID:                        "queue-fanout-2",
		Code:                      "fanout-queue-2",
		Name:                      "Fanout Queue 2",
	...
	qID2, _ := qRepo2.CreateQueue(q2, now)
	q2.ID = qID2
	require.NoError(t, uowQ2.Commit())

	// Bind both queues to the exchange with routing key "user.signup"
	setupTestBinding(t, store, TestFC, TestCFS, exchange, q1, "user.signup", now)
	setupTestBinding(t, store, TestFC, TestCFS, exchange, q2, "user.signup", now)

	// Create OneOff scheduled job targeting Exchange
	uowCreate := db.NewUnitOfWork(store, nil)
	jobRepo, _ := db.NewScheduledJobRepository(uowCreate, idFactory, TestFC, TestCFS)
	fanoutJob := &models.ScheduledJob{
		ID:                             "fanout-job-1",
		TenantID:                       TestCFS,
		TargetType:                     string(models.ScheduledJobTargetExchange),
		TargetID:                       exchange.ID,
		TargetCode:                     exchange.Code,
		RoutingKeyOrPatternOrQueueCode: "user.signup",
		VNamespace:                     "default",
		Content:                        "Fanout Notification Payload",
		Type:                           models.ScheduledJobOneOff,
		RunAfter:                       "1m",
		NextRunAt:                      now,
		State:                          models.ScheduledJobIdle,
	}
	_, err := jobRepo.CreateScheduledJob(fanoutJob, now)
	require.NoError(t, err)
	require.NoError(t, uowCreate.Commit())

	// 1. Dispatch via poller
	uowPoller := db.NewUnitOfWork(store, nil)
	poller := &scheduledJobCommand.ProcessDueScheduledJobsCommand{BatchSize: 100, CF: TestFC, CFS: TestCFS}
	pollerRes := poller.Execute(uowPoller, now)
	require.Empty(t, pollerRes.Error)
	require.NoError(t, uowPoller.Commit())

	resData := pollerRes.Result.(*scheduledJobCommand.ProcessDueScheduledJobsResult)
	assert.Equal(t, 1, resData.ProcessedJobs)
	assert.Equal(t, 2, resData.DispatchedMsgs, "Poller must dispatch 2 messages for fan-out exchange")

	// 2. Dequeue message from queue-1
	uowDeq1 := db.NewUnitOfWork(store, nil)
	deqCmd1 := &queueCommand.DequeueCommand{
		QueueID:       q1.ID,
		JobWorkerID:   "worker-1",
		LeaseDuration: 30 * time.Second,
		CF:            TestFC,
	...
	deqRes1 := deqCmd1.Execute(uowDeq1, now)
	require.Empty(t, deqRes1.Error)
	require.NoError(t, uowDeq1.Commit())
	claimed1 := deqRes1.Result.(queueCommand.DequeueResult)
	require.NotNil(t, claimed1.Lease)
	assert.Equal(t, "fanout-job-1", claimed1.Message.ScheduledJobID)
	require.NotEmpty(t, claimed1.Message.ExecutionID)

	execID := claimed1.Message.ExecutionID

	// Verify trackers exist in DB for this execution ID
	uowVerifyTrackers := db.NewUnitOfWork(store, nil)
	trackerRepoVerify, _ := db.NewScheduledJobTrackerRepository(uowVerifyTrackers, idFactory, TestFC, TestCFS)
	trackers, err := trackerRepoVerify.GetTrackersByExecution(execID, now)
	require.NoError(t, err)
	assert.Len(t, trackers, 2, "There must be 2 trackers for the 2 queues in fan-out")

	// 3. Worker 1 ACKs message on queue-1
	uowAck1 := db.NewUnitOfWork(store, nil)
	ackCmd1 := &queueCommand.AckMessageCommand{
		LeaseID: claimed1.Lease.ID,
		CF:      TestFC,
		CFS:     TestCFS,
	}
	ackRes1 := ackCmd1.Execute(uowAck1, now.Add(1*time.Second))
	require.Empty(t, ackRes1.Error)
	require.NoError(t, uowAck1.Commit())

	// Assert ScheduledJob is STILL in DB (delivered state) because queue-2 is not finished!
	uowCheck1 := db.NewUnitOfWork(store, nil)
	jobRepoCheck1, _ := db.NewScheduledJobRepository(uowCheck1, idFactory, TestFC, TestCFS)
	jobCheck1, err := jobRepoCheck1.GetScheduledJobByID("fanout-job-1", now.Add(1*time.Second))
	require.NoError(t, err)
	require.NotNil(t, jobCheck1, "Scheduled job MUST NOT be deleted when only 1 of 2 queues finishes!")
	assert.Equal(t, models.ScheduledJobDelivered, jobCheck1.State)

	// 4. Dequeue message from queue-2
	uowDeq2 := db.NewUnitOfWork(store, nil)
	deqCmd2 := &queueCommand.DequeueCommand{
		QueueID:       q2.ID,
		JobWorkerID:   "worker-2",
		LeaseDuration: 30 * time.Second,
		CF:            TestFC,
	...
	deqRes2 := deqCmd2.Execute(uowDeq2, now.Add(2*time.Second))
	require.Empty(t, deqRes2.Error)
	require.NoError(t, uowDeq2.Commit())
	claimed2 := deqRes2.Result.(queueCommand.DequeueResult)
	require.NotNil(t, claimed2.Lease)
	assert.Equal(t, execID, claimed2.Message.ExecutionID)

	// 5. Worker 2 ACKs message on queue-2
	uowAck2 := db.NewUnitOfWork(store, nil)
	ackCmd2 := &queueCommand.AckMessageCommand{
		LeaseID: claimed2.Lease.ID,
		CF:      TestFC,
		CFS:     TestCFS,
	}
	ackRes2 := ackCmd2.Execute(uowAck2, now.Add(3*time.Second))
	require.Empty(t, ackRes2.Error)
	require.NoError(t, uowAck2.Commit())

	// Assert ScheduledJob is now DELETED because ALL trackers are finished!
	uowCheck2 := db.NewUnitOfWork(store, nil)
	jobRepoCheck2, _ := db.NewScheduledJobRepository(uowCheck2, idFactory, TestFC, TestCFS)
	deletedJob, err := jobRepoCheck2.GetScheduledJobByID("fanout-job-1", now.Add(3*time.Second))
	require.NoError(t, err)
	assert.Nil(t, deletedJob, "Scheduled job MUST be deleted after all fan-out queues finish!")

	// Verify trackers for execID are cleaned up
	trackerRepoCheck2, _ := db.NewScheduledJobTrackerRepository(uowCheck2, idFactory, TestFC, TestCFS)
	cleanedTrackers, err := trackerRepoCheck2.GetTrackersByExecution(execID, now.Add(3*time.Second))
	require.NoError(t, err)
	assert.Empty(t, cleanedTrackers, "Trackers MUST be cleaned up after job finalization")
}

func TestScheduledJob_ExchangeFanoutExhaustion(t *testing.T) {
	store := newTestPebbleStore(t)
	now := time.Date(2025, 1, 1, 10, 0, 0, 0, time.UTC)
	idFactory := &db.DeterministicIDGeneratorFactory{}

	// Setup exchange and 2 target queues (queue-a MaxAttempts=1, queue-b MaxAttempts=1)
	exchange := setupTestExchange(t, store, TestFC, TestCFS, models.Fanout, now)

	uowQa := db.NewUnitOfWork(store, nil)
	qRepoA, _ := db.NewQueueRepository(uowQa, idFactory, TestFC, TestCFS)
	qa := &models.Queue{
		ID:                        "queue-ex-a",
		Code:                      "ex-queue-a",
		Name:                      "Exhaust Queue A",
		VNamespace:                "default",
		State:                     models.QueueActive,
		Type:                      models.StandardQueue,
		DefaultQueueMessageTTL:    3600,
	...
	qida, _ := qRepoA.CreateQueue(qa, now)
	qa.ID = qida
	require.NoError(t, uowQa.Commit())

	uowQb := db.NewUnitOfWork(store, nil)
	qRepoB, _ := db.NewQueueRepository(uowQb, idFactory, TestFC, TestCFS)
	qb := &models.Queue{
		ID:                        "queue-ex-b",
		Code:                      "ex-queue-b",
		Name:                      "Exhaust Queue B",
		VNamespace:                "default",
		State:                     models.QueueActive,
		Type:                      models.StandardQueue,
		DefaultQueueMessageTTL:    3600,
	...
	qidb, _ := qRepoB.CreateQueue(qb, now)
	qb.ID = qidb
	require.NoError(t, uowQb.Commit())

	setupTestBinding(t, store, TestFC, TestCFS, exchange, qa, "order.created", now)
	setupTestBinding(t, store, TestFC, TestCFS, exchange, qb, "order.created", now)

	// Create OneOff scheduled job
	uowCreate := db.NewUnitOfWork(store, nil)
	jobRepo, _ := db.NewScheduledJobRepository(uowCreate, idFactory, TestFC, TestCFS)
	job := &models.ScheduledJob{
		ID:                             "fanout-exhaust-1",
		...
		TargetID:                       exchange.ID,
		TargetCode:                     exchange.Code,
		RoutingKeyOrPatternOrQueueCode: "order.created",
		VNamespace:                     "default",
		Content:                        "Order Created Payload",
		Type:                           models.ScheduledJobOneOff,
		RunAfter:                       "1m",
		NextRunAt:                      now,
		State:                          models.ScheduledJobIdle,
	}
	_, err := jobRepo.CreateScheduledJob(job, now)
	require.NoError(t, err)
	require.NoError(t, uowCreate.Commit())
	...
	// 2. Dequeue and ACK message on queue-a
	...
	claimedA := deqResA.Result.(queueCommand.DequeueResult)
	uowAckA := db.NewUnitOfWork(store, nil)
	ackCmdA := &queueCommand.AckMessageCommand{
		LeaseID: claimedA.Lease.ID,
		CF:      TestFC,
		CFS:     TestCFS,
	}
	require.Empty(t, ackCmdA.Execute(uowAckA, now.Add(1*time.Second)).Error)
	require.NoError(t, uowAckA.Commit())

	// Job should still exist because queue-b message is still pending
	uowCheck1 := db.NewUnitOfWork(store, nil)
	jobRepoCheck1, _ := db.NewScheduledJobRepository(uowCheck1, idFactory, TestFC, TestCFS)
	jCheck1, _ := jobRepoCheck1.GetScheduledJobByID("fanout-exhaust-1", now.Add(1*time.Second))
	require.NotNil(t, jCheck1)

	// 3. Dequeue message on queue-b (attempts becomes 1 = MaxAttempts)
	...
	deqResB := deqCmdB.Execute(uowDeqB, now)
	require.Empty(t, deqResB.Error)
	require.NoError(t, uowDeqB.Commit())
	claimedB := deqResB.Result.(queueCommand.DequeueResult)

	// Mark lease as delivered to worker stream
	uowMarkB := db.NewUnitOfWork(store, nil)
	markCmdB := &queueCommand.MarkLeaseDeliveredCommand{
		LeaseID: claimedB.Lease.ID,
		CF:      TestFC,
		CFS:     TestCFS,
	}
	require.Empty(t, markCmdB.Execute(uowMarkB, now).Error)
	require.NoError(t, uowMarkB.Commit())

	// Process expired lease on queue-b -> reaches MaxAttempts and is deleted / exhausted!
	tExpire := now.Add(10 * time.Second)
	uowExpire := db.NewUnitOfWork(store, nil)
	procExpiredCmd := &queueCommand.ProcessExpiredLeasesCommand{
		Limit: 100,
		CF:    TestFC,
		CFS:   TestCFS,
	}
	expireRes := procExpiredCmd.Execute(uowExpire, tExpire)
	require.Empty(t, expireRes.Error)
	require.NoError(t, uowExpire.Commit())

	// Now ALL trackers are finished (1 completed, 1 exhausted). Job MUST be finalized (deleted)!
	uowCheck2 := db.NewUnitOfWork(store, nil)
	jobRepoCheck2, _ := db.NewScheduledJobRepository(uowCheck2, idFactory, TestFC, TestCFS)
	deletedJob, err := jobRepoCheck2.GetScheduledJobByID("fanout-exhaust-1", tExpire)
	require.NoError(t, err)
	assert.Nil(t, deletedJob, "Job MUST be finalized when remaining trackers are exhausted!")
}
